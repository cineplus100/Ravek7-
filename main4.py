...
USUARIOS_COOLDOWN = {}  # {user_id: datetime}
LOJA_ITENS = {
    "cor-neon": 150,
    "tag-vip": 300,
...
XP_POR_MENSAGEM = 5
XP_PARA_NIVEL = 100  # cada 100 XP = 1 nível

# Relatório
ENVIADO_HOJE = set()  # {(guild_id, h, m)}
...
async def on_ready():
    ...
    try:
        ...
        print(f"✅ Comandos de slash sincronizados ({len(synced)})")
...
async def banir(interaction, membro: discord.Member, motivo: str = None):
    ...
    if membro.id in DONOS_IDS:
        return await interaction.response.send_message("❌ Não pode banir o dono.", ephemeral=True)
...
async def periodic_relatorio():
    agora = datetime.now(timezone.utc) - timedelta(hours=3)  # Bahia UTC-3
...
async def iniciar_bot():
    ...
    if not token:
        raise ValueError("⚠️ Defina a variável de ambiente DISCORD_TOKEN")
    while True:
        try:
